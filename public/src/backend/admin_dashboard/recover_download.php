...
if (isset($_POST['id'], $_POST['key_name'])) {
    ...
    $stmt = $conn->prepare("SELECT file_name, original_name FROM download_archive WHERE id=? AND key_name=?");
        ...
        if ($currentFile) {
            ...
            if (file_exists($currentPath)) {
                ...
                if (copy($currentPath, $archiveDir . $archivedName)) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO download_archive (key_name, file_name, original_name) VALUES (?, ?, ?)");
...
header("Location: ../../components/admin_dashboard/download_archieve.php");
exit;