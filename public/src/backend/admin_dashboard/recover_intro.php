...
if (isset($_POST['id'], $_POST['key_name'])) {
    ...
    $stmt = $conn->prepare("SELECT file_name, original_name FROM intro_archive WHERE id=? AND key_name=?");
        ...
        if ($currentFile) {
            ...
            if (file_exists($currentPath)) {
                ...
                if (copy($currentPath, $archiveDir . $archivedName)) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO intro_archive (key_name, file_name, original_name) VALUES (?, ?, ?)");
...
header("Location: ../../components/admin_dashboard/intro_archieve.php");
exit;